...
class SessionModel {
    ...
    `;
        ...
        try {
            ...
            if (error.code === '23505') { // Unique violation
                throw new shared_1.SessionError('SESSION_EXISTS', `Session ${sessionData.session_id} already exists`, 409);
            }
            throw error;
        }
    }
    ...
    async update(sessionId, updates) {
        ...
        if (!session) {
            throw new shared_1.SessionError('SESSION_NOT_FOUND', `Session ${sessionId} not found`, 404);
        }
        const fields = [];
        const values = [];
        let paramCount = 1;
    ...
    async addDetectedClass(sessionId, className) {
        const query = `
      UPDATE sessions 
    ...
    `;
        await database_1.db.query(query, [sessionId, className]);
    }
    async query(filters) {
        let baseQuery = `
      FROM sessions s
    ...
    `;
        ...
        const countResult = await database_1.db.query(countQuery, values);
    ...
    `;
        const result = await database_1.db.query(selectQuery, [...values, limit, offset]);
