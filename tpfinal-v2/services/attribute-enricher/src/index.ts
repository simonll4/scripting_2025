...
app.post('/api/enrich', async (req: any, res: any) => {
  try {
    const enrichmentRequest: EnrichmentRequest = req.body;
...
app.post('/api/enrich/upload', upload.single('image'), async (req: any, res: any) => {
  try {
...
app.post('/api/enrich/batch', async (req: any, res: any) => {
  try {
    const { requests }: { requests: EnrichmentRequest[] } = req.body;
    ...
    if (successfulResults.length > 0) {
      await Promise.all(
        successfulResults.map(result => updateDetectionAttributes(result))
      );
    }
...
app.post('/api/colors/palette', upload.single('image'), async (req: any, res: any) => {
  try {
    if (!req.file) {
      return res.status(400).json({
        error: 'Image file is required'
...
async function processEnrichment(request: EnrichmentRequest): Promise<EnrichmentResult> {
  const startTime = Date.now();
  ...
  try {
    ...
    if (request.frameUrl) {
      ...
      const response = await fetch(request.frameUrl);
      if (!response.ok) {
...
async function processImageBuffer(
  imageBuffer: Buffer, 
  request: Pick<EnrichmentRequest, 'sessionId' | 'detectionId'>
): Promise<Omit<EnrichmentResult, 'processingTime'>> {
  // Get image stats
...
async function extractColorPalette(imageBuffer: Buffer): Promise<ColorInfo[]> {
...
async function calculateAverageColor(imageBuffer: Buffer): Promise<ColorInfo> {
...
function calculateColorDistribution(colors: ColorInfo[]): { [colorName: string]: number } {
  const distribution: { [colorName: string]: number } = {};
...
function getColorFamily(hsl: [number, number, number]): string {
  const [h, s, l] = hsl;
...
async function updateDetectionAttributes(result: EnrichmentResult): Promise<void> {
  if (result.error) return;
  
  try {
    const response = await fetch(`${CONFIG.SESSION_STORE_URL}/api/detections/${result.detectionId}`, {
      method: 'PATCH',
      headers: {
