...
import { CameraCapture, FrameData } from './camera/CameraCapture';
import { InferenceEngine } from './inference/InferenceEngine';
import { StreamProcessor } from './streaming/StreamProcessor';
import { SessionStoreClient } from './services/SessionStoreClient';
import { ObjectStorageClient } from './services/ObjectStorageClient';
...
export class EdgeAgent extends EventEmitter {
  ...
  private sessionStore: SessionStoreClient;
  ...
  private relevantDetectionCount: number = 0;
  private streamingThreshold: number = 3; // Iniciar streaming después de 3 detecciones relevantes

  // Frame processing
  private currentFrameBuffer: Buffer | null = null;
  private currentFrameTimestamp: number = 0;

  // Buffers para datos de sesión
  private sessionDetections: Detection[] = [];
  private sessionFrames: FrameAnnotation[] = [];
  private bestFrame: { buffer: Buffer; score: number; timestamp: number } | null = null;

  // Timers
  private postRollTimer: NodeJS.Timeout | null = null;

  constructor() {
    super();

    // EdgeAgent como orquestador central - maneja la cámara y distribuye frames
    this.camera = new CameraCapture(CONFIG.DEVICE_PATH);
    this.inference = new InferenceEngine();
    this.streamProcessor = new StreamProcessor();
    this.sessionStore = new SessionStoreClient();
  ...
  private async handleInferenceDetection(detectionData: any): Promise<void> {
    try {
      ...
      if (detection.score >= CONFIG.CONFIDENCE_THRESHOLD && 
          CONFIG.CLASSES_OF_INTEREST.includes(detection.class)) {
  ...
  private async handleDetections(detections: any[]): Promise<void> {
    ...
    try {
      ...
      const convertedDetections: Detection[] = detections.map(det => ({
        class: det.class,
      ...
      // Filter relevant detections
      const relevantDetections = convertedDetections.filter(d => 
        d.score >= CONFIG.CONFIDENCE_THRESHOLD && 
        CONFIG.CLASSES_OF_INTEREST.includes(d.class)
      );
  ...
  private async handleRelevantDetections(
    ...
    if (this.currentSessionId) {
      ...
      for (const [className, classDetections] of detectionsByClass) {
        ...
        if (!recentDetection && frameBuffer) {
          ...
          const frameUrl = await this.objectStorage.saveFrame(
          ...
          const bestDetection = classDetections.reduce((best, current) => 
  ...
  private async startNewSession(timestamp: number): Promise<void> {
    const sessionId = generateSessionId();
    this.currentSessionId = sessionId;
    this.sessionStartTime = timestamp;
    this.currentState = SessionState.OPEN;
  ...
  private async startStreaming(): Promise<void> {
    ...
    try {
      ...
      const streamUrl = this.streamProcessor.startSessionStream(this.currentSessionId, CONFIG.DEVICE_PATH);
  ...
  private async closeCurrentSession(timestamp: number): Promise<void> {
  ...
  private async sendUniqueDetections(detections: Detection[]): Promise<void> {
    if (!this.currentSessionId || detections.length === 0) return;
  ...
  private async sendDetectionsBatch(detections: Detection[]): Promise<void> {
    if (!this.currentSessionId || detections.length === 0) return;
  ...
  private async testSystemDependencies(): Promise<void> {
    console.log('🔍 Testing system dependencies...');

    // Test GStreamer
    const gstAvailable = await StreamProcessor.testGStreamerAvailability();
    if (!gstAvailable) {
    ...
    // Test video device
    const deviceAvailable = await CameraCapture.testDevice(CONFIG.DEVICE_PATH);
    if (!deviceAvailable) {
      console.warn(`⚠️ Video device ${CONFIG.DEVICE_PATH} not available, using test source`);
    } else {
      console.log(`✅ Video device ${CONFIG.DEVICE_PATH} available`);
    }
  }
  ...
  async forceStartSession(): Promise<string> {
    if (this.currentState !== SessionState.IDLE) {
